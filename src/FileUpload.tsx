...
const FileUpload: React.FC = () => {
	...
	importer
		...
		.then((result) => {
			...
			for (const outcome of result.value) {
				if (outcome.isRight()) {
					outcome.value.category;
					outcome.value.comment;
					outcome.value.date;
					outcome.value.value;
					outcome.value.currency;
				} else {
					for (let column of outcome.value.fields) {
						row[column] = outcome.value.fields?.[column]; // render the row's errors
						outcome.value.message; // render the common error message
					}
				}
			}
		});
	...
	const handleContinue = () => {
		...
		reader.onload = async (e) => {
			...
			try {
				const parsed = await importer
					.load(selectedBank)
					.then((impl) => impl.run(binary));
					...
					{current === 0 && (
						<div className={styles.selectContainer}>
							<Select
								...
								onChange={(value: importer.Bank) => setSelectedBank(value)}
								style={{ width: 128 }}
							>
							...
							<Button
								...
								onClick={async () => {
									...
									const exported = await exporter
										.load("json")
										.then((impl) => impl.run(scanIterable as any));
									if (exported.isLeft()) {
